...
WidgetCommonRareWords::WidgetCommonRareWords(const QSqlDatabase &db, int animType, QWidget *parent)
    : QWidget(parent),
      m_db(&db),
...
WidgetCommonRareWords::~WidgetCommonRareWords()
{

}

void WidgetCommonRareWords::resizeEvent(QResizeEvent *evt)
{
    if (!m_isBeingOpened) {
...
void WidgetCommonRareWords::GenerateMainLayout(int animType)
{
...
void WidgetCommonRareWords::on_visualizeButton_clicked()
{
    ...
    //QRegExp rgxMarkupNotes("{TN}.*{tn}|{.{1,4}}|,|\\.|:|;|\\?|!|\\(|\\)");
    QString testStr = "(And this is the <i>sample</i> sentence) {TN}sample note{tn}."
                      "And a question?! {OT}Also{ot}, [another example]; how about this?"
                      "";

    QRegularExpression rgxMarkupNotes("{TN}.*{tn}|{.{1,4}}|<.{1,2}>|[()]|[.]|[?]|!|,|:|;");
    qDebug() << testStr;
    testStr.remove(rgxMarkupNotes);
    qDebug() << testStr;
    ...
    if (query.exec(queryString)) {
        while (query.next()) {
            ...
            scripture.remove(rgxMarkupNotes);
            //qDebug() << scripture;
            QStringList separateWords = scripture.split(" ");
...
void WidgetCommonRareWords::plotResults(const QHash<QString, int> &uniqueWords)
{
...
void WidgetCommonRareWords::enableVisualizeButton()
{
